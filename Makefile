# Aware — build/run wrapper around xcodebuild. Mirrors the rest of our iOS apps
# (clink-ios in particular) so the muscle memory carries across.
#
...
APP_NAME       := Aware
SCHEME         := Aware
BUNDLE_ID      := ltd.anti.aware

PROJECT        := Aware.xcodeproj
BUILD_DIR      := build
...
# Device selection. Defaults to the lambda-ios phone (this dev machine's test
# device); other paired iPhones — e.g. a remote unit — would otherwise win the
# name-less pick just by sorting first. Override with DEVICE=<udid> or
# DEVICE_NAME="My iPhone" (DEVICE_NAME= to pick any).
DEVICE         ?=
DEVICE_NAME    ?= OSV1

# App Store Connect distribution. Same API key the other apps use; the .p8 lives
# outside the repo.
...
help:
	...
	@echo "  make device         — build + install + launch Aware on the paired iPhone"
	@echo "  make device-install — build + install (no launch)"
	@echo "  make device-launch  — just relaunch the installed app"
	@echo ""
	@echo "Distribution:"
	@echo "  make upload-asc — archive + package + validate + upload to App Store Connect"
	@echo ""
	@echo "Overrides:"
...
# Bake CHANGELOG.md into Sources/Aware/Changelog.generated.swift so Profile →
# About → Changelog can render it in-app. The output depends on the source
# markdown + the generator, so editing either re-bakes on the next build (the
# `build*` targets list $(CHANGELOG_OUT) as a prerequisite).
CHANGELOG_SRC := CHANGELOG.md
CHANGELOG_GEN := Tools/GenerateChangelog.swift
CHANGELOG_OUT := Sources/Aware/Changelog.generated.swift
...
# ============================================================
# App Store Connect distribution. Signing is automatic:
...
#
#   make archive      — Release archive of the app
